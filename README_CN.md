...
Sub2API 支持 [MiniMax](https://www.minimax.io/) 账户，通过 API Key 接入。MiniMax 使用兼容 Anthropic 的 API 接口，支持 MiniMax-M2.5 和 MiniMax-M2.5-highspeed 模型。

### 配置方式

1. 在管理面板中使用 MiniMax API Key 创建 MiniMax 账号
2. 默认 Base URL 为 `https://api.minimax.io/anthropic`
3. 如果启用了 URL 白名单，请将 `api.minimax.io` 添加到 `config.yaml` 的 `upstream_hosts` 中
...
使用 MiniMax 模型（如 `MiniMax-M2.5`）的请求将自动路由到 MiniMax 账号。
