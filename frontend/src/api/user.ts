...
function extractUserFromMutationResponse(
  payload: ProfileMutationResponse,
): User | null {
  if (!payload) {
    return null;
  }
...
async function requestFirstSupported<T>(
...
export async function bindAccount(
  ...
  if (provider === "email") {
    ...
    return requestFirstSupported([
      () =>
        normalizeProfileMutation(
          apiClient.post<ProfileMutationResponse>(
      ...
      () =>
        normalizeProfileMutation(
          apiClient.post<ProfileMutationResponse>("/user/bindings/email", {
            email: emailPayload.email,
            verify_code: emailPayload.verifyCode,
            password: emailPayload.password,
          }),
  ...
  return requestFirstSupported([
    ...
    () =>
      normalizeProfileMutation(
        apiClient.post<ProfileMutationResponse>(`/user/bindings/${provider}`, {
          pending_auth_token: pendingAuthToken,
          pending_oauth_token: pendingAuthToken,
        }),
      ),
    () =>
      normalizeProfileMutation(
        apiClient.post<ProfileMutationResponse>(`/user/binding/${provider}`, {
          pending_auth_token: pendingAuthToken,
          pending_oauth_token: pendingAuthToken,
        }),
      ),
...
export async function setAccountBindingAdoptionDecision(
  ...
  await requestFirstSupported([
    () =>
      apiClient
        .post(`/user/account-bindings/${provider}/adoption-decision`, {
          pending_auth_token: pendingAuthToken,
          pending_oauth_token: pendingAuthToken,
    ...
    () =>
      apiClient
        .post(`/user/bindings/${provider}/adoption-decision`, {
          pending_auth_token: pendingAuthToken,
          pending_oauth_token: pendingAuthToken,
          adopt_display_name: adoptDisplayName,
          adopt_avatar: adoptAvatar,
...
export async function unbindAccount(
  ...
  return requestFirstSupported([
    () =>
      normalizeProfileMutation(
        apiClient.delete<ProfileMutationResponse>(
          `/user/account-bindings/${provider}`,
        ),
      ),
    () =>
      normalizeProfileMutation(
        apiClient.delete<ProfileMutationResponse>(`/user/bindings/${provider}`),
      ),
    () =>
      normalizeProfileMutation(
        apiClient.delete<ProfileMutationResponse>(`/user/binding/${provider}`),
      ),
    () =>
      normalizeProfileMutation(
        apiClient.delete<ProfileMutationResponse>("/user/account-bindings", {
          data: { provider },
        }),
      ),
    () =>
      normalizeProfileMutation(
        apiClient.delete<ProfileMutationResponse>("/user/bindings", {
          data: { provider },
        }),
      ),
  ]);
