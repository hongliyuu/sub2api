            ...
            <template v-else>
              ...
              <div class="space-y-4 border-t border-gray-100 pt-4 dark:border-dark-700">
                <div class="flex items-center justify-between">
                  <h3 class="text-sm font-medium text-gray-900 dark:text-white">
                    {{ t('admin.settings.extremePerformance.runtimeStatus') }}
                  </h3>
                  <button
                    type="button"
                    class="btn btn-secondary btn-sm"
                    :disabled="extremePerformanceStatusLoading"
                    @click="loadExtremePerformanceStatus"
                  >
                    {{ t('common.refresh') }}
                  </button>
                </div>

                <div v-if="extremePerformanceStatusLoading" class="text-sm text-gray-500">
                  {{ t('common.loading') }}
                </div>
                <div v-else class="grid grid-cols-1 gap-3 md:grid-cols-3">
                  <div
                    v-for="item in extremePerformanceStatus"
                    :key="item.platform"
                    class="rounded-lg border border-gray-200 p-3 text-sm dark:border-dark-600"
                  >
                    <div class="font-medium text-gray-900 dark:text-white">{{ item.platform }}</div>
                    <div class="mt-2 space-y-1 text-gray-600 dark:text-gray-300">
                      <div>{{ t('admin.settings.extremePerformance.targetSize') }}: {{ item.target_size }}</div>
                      <div>{{ t('admin.settings.extremePerformance.currentSize') }}: {{ item.current_size }}</div>
                      <div>{{ t('admin.settings.extremePerformance.coldCandidates') }}: {{ item.cold_candidates ?? 0 }}</div>
                      <div>{{ t('admin.settings.extremePerformance.version') }}: {{ item.version }}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-wrap justify-end gap-3 border-t border-gray-100 pt-4 dark:border-dark-700">
                <button
                  type="button"
                  class="btn btn-secondary btn-sm"
                  :disabled="extremePerformanceActionLoading !== null"
                  @click="triggerExtremePerformanceRefill"
                >
                  {{ extremePerformanceActionLoading === 'refill' ? t('common.loading') : t('admin.settings.extremePerformance.triggerRefill') }}
                </button>
                <button
                  type="button"
                  class="btn btn-secondary btn-sm"
                  :disabled="extremePerformanceActionLoading !== null"
                  @click="triggerExtremePerformanceRebuild"
                >
                  {{ extremePerformanceActionLoading === 'rebuild' ? t('common.loading') : t('admin.settings.extremePerformance.triggerRebuild') }}
                </button>
                <button
                  type="button"
...
const extremePerformanceStatusLoading = ref(false)
const extremePerformanceActionLoading = ref<'refill' | 'rebuild' | null>(null)
const extremePerformanceStatus = ref<Array<{
  platform: string
  target_size: number
  current_size: number
...
async function loadExtremePerformanceStatus() {
  extremePerformanceStatusLoading.value = true
  try {
    const status = await adminAPI.settings.getExtremePerformanceStatus()
    extremePerformanceStatus.value = status.platforms || []
  } catch (error: any) {
    console.error('Failed to load extreme performance status:', error)
  } finally {
    extremePerformanceStatusLoading.value = false
  }
}

async function triggerExtremePerformanceRefill() {
  extremePerformanceActionLoading.value = 'refill'
  try {
    const result = await adminAPI.settings.triggerExtremePerformanceRefill()
    await loadExtremePerformanceStatus()
    appStore.showSuccess(result.message || t('admin.settings.extremePerformance.refillTriggered'))
  } catch (error: any) {
    appStore.showError(
      t('admin.settings.extremePerformance.actionFailed') + ': ' + (error.message || t('common.unknownError'))
    )
  } finally {
    extremePerformanceActionLoading.value = null
  }
}

async function triggerExtremePerformanceRebuild() {
  extremePerformanceActionLoading.value = 'rebuild'
  try {
    const result = await adminAPI.settings.triggerExtremePerformanceRebuild()
    await loadExtremePerformanceStatus()
    appStore.showSuccess(result.message || t('admin.settings.extremePerformance.rebuildTriggered'))
  } catch (error: any) {
    appStore.showError(
      t('admin.settings.extremePerformance.actionFailed') + ': ' + (error.message || t('common.unknownError'))
    )
...
async function saveExtremePerformanceSettings() {
  ...
  try {
    ...
    await Promise.all([adminSettingsStore.fetch(true), loadExtremePerformanceStatus()])
