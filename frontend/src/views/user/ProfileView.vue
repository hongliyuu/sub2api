...
import { useAppStore } from '@/stores/app'; import AppLayout from '@/components/layout/AppLayout.vue'
...
onMounted(async () => { try { const s = await appStore.fetchPublicSettings(); if (!s) return; contactInfo.value = s.contact_info || ''; balanceLowNotifyEnabled.value = s.balance_low_notify_enabled ?? false; systemDefaultThreshold.value = s.balance_low_notify_threshold ?? 0 } catch (error) { console.error('Failed to load settings:', error) } })
