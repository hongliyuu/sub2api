<template>
  ...
  <div v-else class="space-y-6">
    <div v-if="loading" class="flex items-center justify-center py-12"><LoadingSpinner /></div>
    <template v-else-if="stats">
      <UserDashboardStats :stats="stats" :balance="adminUser?.balance || 0" :is-simple="adminUser?.run_mode === 'simple'" />
      <UserDashboardCharts v-model:startDate="startDate" v-model:endDate="endDate" v-model:granularity="granularity" :loading="loadingCharts" :trend="trendData" :models="modelStats" @dateRangeChange="loadCharts" @granularityChange="loadCharts" />
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2"><UserDashboardRecentUsage :data="recentUsage" :loading="loadingUsage" /></div>
        <div class="lg:col-span-1"><UserDashboardQuickActions /></div>
...
const loadStats = async () => { loading.value = true; try { if (props.adminUserId) { [stats.value, adminUser.value] = await Promise.all([adminUserViewAPI.getDashboardStats(props.adminUserId), adminUsersAPI.getById(props.adminUserId)]) } else { await authStore.refreshUser(); stats.value = await usageAPI.getDashboardStats() } } catch (error) { console.error('Failed to load dashboard stats:', error) } finally { loading.value = false } }
