    ...
    <div class="mx-auto max-w-2xl">
      <div class="card p-6">
        <h1 class="text-lg font-semibold text-gray-900 dark:text-white">
          {{ t('auth.wechat.callbackTitle') }}
        </h1>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
          {{ errorMessage || t('auth.wechat.callbackProcessing') }}
        </p>
...
onMounted(async () => {
  ...
  if (!openid) {
    errorMessage.value = t('auth.wechat.callbackMissingOpenId')
    return
  }
