  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div v-if="showDailyQuota" class="flex items-center gap-1">
      <span
        ...
        :title="dailyQuotaTooltip"
      >
        <span class="font-semibold opacity-70">D</span>
        <span class="font-mono">${{ formatCost(account.quota_daily_used) }}</span>
        <span class="text-gray-400 dark:text-gray-500">/</span>
        <span class="font-mono">${{ formatCost(account.quota_daily_limit) }}</span>
      </span>
    </div>
    ...
    <div v-if="showWeeklyQuota" class="flex items-center gap-1">
      <span
        ...
        :title="weeklyQuotaTooltip"
      >
        <span class="font-semibold opacity-70">W</span>
        <span class="font-mono">${{ formatCost(account.quota_weekly_used) }}</span>
        <span class="text-gray-400 dark:text-gray-500">/</span>
        <span class="font-mono">${{ formatCost(account.quota_weekly_limit) }}</span>
      </span>
    </div>
...
const dailyQuotaClass = computed(() =>
  quotaBadgeClass(props.account.quota_daily_used ?? 0, props.account.quota_daily_limit ?? 0)
)

const dailyQuotaTooltip = computed(() =>
  quotaBadgeTooltip(props.account.quota_daily_used ?? 0, props.account.quota_daily_limit ?? 0)
)
...
const weeklyQuotaClass = computed(() =>
  quotaBadgeClass(props.account.quota_weekly_used ?? 0, props.account.quota_weekly_limit ?? 0)
)

const weeklyQuotaTooltip = computed(() =>
  quotaBadgeTooltip(props.account.quota_weekly_used ?? 0, props.account.quota_weekly_limit ?? 0)
)
...
const totalQuotaClass = computed(() =>
  quotaBadgeClass(props.account.quota_used ?? 0, props.account.quota_limit ?? 0)
)

const totalQuotaTooltip = computed(() =>
  quotaBadgeTooltip(props.account.quota_used ?? 0, props.account.quota_limit ?? 0)
)

// ====== 配额公共逻辑 ======
function quotaBadgeClass(used: number, limit: number): string {
  if (limit <= 0) return ''
  if (used >= limit) {
    return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
  }
...
function quotaBadgeTooltip(used: number, limit: number): string {
  if (limit <= 0) return ''
  if (used >= limit) {
