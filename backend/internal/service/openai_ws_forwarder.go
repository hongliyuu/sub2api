...
func (s *OpenAIGatewayService) buildOpenAIWSCreatePayload(reqBody map[string]any, account *Account) map[string]any {
	...
	normalizeCodexReasoningForUpstream(payload)
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		next, reasoningChanged, setErr := normalizeOpenAIReasoningForUpstreamBody(normalized)
		if setErr != nil {
			return openAIWSClientPayload{}, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", setErr)
		}
		if reasoningChanged {
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string) (*OpenAIForwardResult, error) {
		...
		turnHasFunctionCallOutput := HasToolCallOutputInBody(payload)
		eventCount := 0
		tokenEventCount := 0
		terminalEventCount := 0
		firstEventType := ""
	...
	for {
		...
		hasFunctionCallOutput := HasToolCallOutputInBody(currentPayload)
