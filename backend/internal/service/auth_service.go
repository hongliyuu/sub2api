...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affCode string) (string, *User, error) {
	...
	// 处理邀请返利（affCode 是注册请求中的推荐码字段）
	if affCode != "" && s.referralService != nil {
		if err := s.referralService.BindInviter(ctx, affCode, user.ID); err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Failed to bind referral for user %d: %v", user.ID, err)
		}
	}
	// 应用优惠码（如果提供且功能已启用）
