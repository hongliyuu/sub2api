...
type UserSubscriptionQuery struct {
	...
	withQuotaEvents    *UserSubscriptionQuotaEventQuery
...
// QueryQuotaEvents chains the current query on the "quota_events" edge.
func (_q *UserSubscriptionQuery) QueryQuotaEvents() *UserSubscriptionQuotaEventQuery {
	query := (&UserSubscriptionQuotaEventClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserSubscriptionQuery) Clone() *UserSubscriptionQuery {
	...
	return &UserSubscriptionQuery{
		...
		withQuotaEvents:    _q.withQuotaEvents.Clone(),
...
// WithQuotaEvents tells the query-builder to eager-load the nodes that are connected to
// the "quota_events" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserSubscriptionQuery) WithQuotaEvents(opts ...func(*UserSubscriptionQuotaEventQuery)) *UserSubscriptionQuery {
	query := (&UserSubscriptionQuotaEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withQuotaEvents = query
	return _q
}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	...
	if query := _q.withQuotaEvents; query != nil {
		if err := _q.loadQuotaEvents(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.QuotaEvents = []*UserSubscriptionQuotaEvent{} },
			func(n *UserSubscription, e *UserSubscriptionQuotaEvent) {
				n.Edges.QuotaEvents = append(n.Edges.QuotaEvents, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserSubscriptionQuery) loadQuotaEvents(ctx context.Context, query *UserSubscriptionQuotaEventQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *UserSubscriptionQuotaEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserSubscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usersubscriptionquotaevent.FieldUserSubscriptionID)
	}
	query.Where(predicate.UserSubscriptionQuotaEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.QuotaEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
