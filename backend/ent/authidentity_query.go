...
func (_q *AuthIdentityQuery) QueryAdoptionDecision() *IdentityAdoptionDecisionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, authidentity.AdoptionDecisionTable, authidentity.AdoptionDecisionColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *AuthIdentityQuery) WithAdoptionDecision(opts ...func(*IdentityAdoptionDecisionQuery)) *AuthIdentityQuery {
	query := (&IdentityAdoptionDecisionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AuthIdentityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuthIdentity, error) {
	...
	if query := _q.withAdoptionDecision; query != nil {
		if err := _q.loadAdoptionDecision(ctx, query, nodes, nil,
			func(n *AuthIdentity, e *IdentityAdoptionDecision) { n.Edges.AdoptionDecision = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuthIdentityQuery) loadAdoptionDecision(ctx context.Context, query *IdentityAdoptionDecisionQuery, nodes []*AuthIdentity, init func(*AuthIdentity), assign func(*AuthIdentity, *IdentityAdoptionDecision)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*AuthIdentity)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.IdentityAdoptionDecision(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(authidentity.AdoptionDecisionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
