...
type PendingAuthSession struct {
	...
	// UpstreamIdentityPayload holds the value of the "upstream_identity_payload" field.
	UpstreamIdentityPayload map[string]interface{} `json:"upstream_identity_payload,omitempty"`
...
func (*PendingAuthSession) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case pendingauthsession.FieldUpstreamIdentityPayload:
			values[i] = new([]byte)
		case pendingauthsession.FieldID, pendingauthsession.FieldTargetUserID:
			values[i] = new(sql.NullInt64)
		case pendingauthsession.FieldIntent, pendingauthsession.FieldProviderType, pendingauthsession.FieldProviderKey, pendingauthsession.FieldProviderSubject, pendingauthsession.FieldStateHash, pendingauthsession.FieldNonce, pendingauthsession.FieldRedirectTo:
			values[i] = new(sql.NullString)
		case pendingauthsession.FieldCreatedAt, pendingauthsession.FieldUpdatedAt, pendingauthsession.FieldExpiresAt, pendingauthsession.FieldConsumedAt:
	...
	for i := range columns {
		...
		case pendingauthsession.FieldUpstreamIdentityPayload:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field upstream_identity_payload", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.UpstreamIdentityPayload); err != nil {
					return fmt.Errorf("unmarshal field upstream_identity_payload: %w", err)
				}
			}
...
func (_m *PendingAuthSession) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("upstream_identity_payload=")
	builder.WriteString(fmt.Sprintf("%v", _m.UpstreamIdentityPayload))
	builder.WriteString(", ")
