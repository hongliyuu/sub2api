...
func (_c *IdentityAdoptionDecisionCreate) check() error {
	...
	if _, ok := _c.mutation.IdentityID(); !ok {
		return &ValidationError{Name: "identity_id", err: errors.New(`ent: missing required field "IdentityAdoptionDecision.identity_id"`)}
	}
	if _, ok := _c.mutation.AdoptDisplayName(); !ok {
	...
	if len(_c.mutation.IdentityIDs()) == 0 {
		return &ValidationError{Name: "identity", err: errors.New(`ent: missing required edge "IdentityAdoptionDecision.identity"`)}
	}
	return nil
}
