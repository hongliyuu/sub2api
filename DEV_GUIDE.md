...
| 数据库凭据 | user=`nbapi`, password=`nbapi`, dbname=`nbapi` |
...
psql -U nbapi -h 127.0.0.1 -d nbapi -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER nbapi WITH PASSWORD 'nbapi';
...
```bash
# 连接数据库
psql -U nbapi -h 127.0.0.1 -d nbapi
...
# 执行 SQL 文件
psql -U nbapi -h 127.0.0.1 -d nbapi -f migration.sql
```
...
- [上游仓库](https://github.com/Wei-Shaw/nbapi)
