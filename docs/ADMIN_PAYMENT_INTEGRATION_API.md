...
### 目标
本文档用于对接外部支付系统（如 `nbapipay`）与 nbAPI 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
```json
{
  ...
  "value": 100.0,
  "user_id": 123,
  "notes": "nbapipay order: cm1234567890"
}
```
...
当 nbAPI 打开 `purchase_subscription_url` 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
- `ui_mode`（固定 `embedded`）
...
### 6) `doc_url` 配置建议
- 查看链接：`https://github.com/Wei-Shaw/nbapi/blob/main/ADMIN_PAYMENT_INTEGRATION_API.md`
- 下载链接：`https://raw.githubusercontent.com/Wei-Shaw/nbapi/main/ADMIN_PAYMENT_INTEGRATION_API.md`
...
### Purpose
This document describes the minimal nbAPI Admin API surface for external payment integrations (for example, `nbapipay`), including:
...
When nbAPI opens `purchase_subscription_url`, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
...
- View URL: `https://github.com/Wei-Shaw/nbapi/blob/main/ADMIN_PAYMENT_INTEGRATION_API.md`
- Download URL: `https://raw.githubusercontent.com/Wei-Shaw/nbapi/main/ADMIN_PAYMENT_INTEGRATION_API.md`
