#!/bin/bash
#
# nbAPI Installation Script
# nbAPI 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/Wei-Shaw/nbapi/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="Wei-Shaw/nbapi"
INSTALL_DIR="/opt/nbapi"
SERVICE_NAME="nbapi"
SERVICE_USER="nbapi"
CONFIG_DIR="/etc/nbapi"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="nbAPI 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 nbAPI 服务："
    ...
    ["not_installed"]="nbAPI 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 nbAPI。"
    ...
    ["uninstall_complete"]="nbAPI 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 nbAPI"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 nbAPI"
    ...
    ["server_config_desc"]="配置 nbAPI 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="nbAPI Installation Script"
    ...
    # Completion
    ["install_complete"]="nbAPI installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start nbAPI service:"
    ...
    ["not_installed"]="nbAPI is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove nbAPI from your system."
    ...
    ["uninstall_complete"]="nbAPI has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install nbAPI"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove nbAPI"
    ...
    ["server_config_desc"]="Configure nbAPI server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/nbapi" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/nbapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="nbapi_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/nbapi" "$INSTALL_DIR/nbapi"
    chmod +x "$INSTALL_DIR/nbapi"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/nbapi"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/nbapi.service << EOF
[Unit]
Description=nbAPI - AI API Gateway Platform
Documentation=https://github.com/Wei-Shaw/nbapi
...
User=nbapi
Group=nbapi
WorkingDirectory=/opt/nbapi
ExecStart=/opt/nbapi/nbapi
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=nbapi
...
enable_autostart() {
    ...
    if systemctl enable nbapi 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status nbapi"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u nbapi -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart nbapi"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop nbapi"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/nbapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet nbapi; then
        print_info "$(msg 'stopping_service')"
        systemctl stop nbapi
    fi

    # Backup current binary
    cp "$INSTALL_DIR/nbapi" "$INSTALL_DIR/nbapi.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/nbapi.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/nbapi"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start nbapi

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: nbAPI must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/nbapi" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="nbapi.backup.${current_version}"
        else
            backup_name="nbapi.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/nbapi" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop nbapi 2>/dev/null || true
    systemctl disable nbapi 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/nbapi.service
    systemctl daemon-reload
